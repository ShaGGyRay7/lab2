...
// ["ІВ-72": 6.2844447, "ІВ-73": 6.466667, "ІВ-71": 6.677778]
...
class CoordinatePM {
    ...
    func midpoint(another: CoordinatePM) -> CoordinatePM? {
        if self.direction != another.direction {
            return nil
        } else {
            let coordinate = try! CoordinatePM(
    ...
    static func midpoint(one: CoordinatePM, another: CoordinatePM) -> CoordinatePM? {
...
let c1 = CoordinatePM(direction: .north)
let c2 = try! CoordinatePM(direction: .north,
                           degree: 50,
                           minute: 13,
                           second: 22)
print(c1.toString1())
...
print(m1.toString1())
let m2 = try! CoordinatePM.midpoint(one: c1, another: c2)!
print(m2.toString2())
